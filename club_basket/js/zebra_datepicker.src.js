...
(function(factory) {
    ...
    $.Zebra_DatePicker = function(element, options) {
        ...
        var defaults = {
			...
			plugin.settings.header_captions.years = 'Tanggal lahir';
                // manage header caption and enable/disable navigation buttons if necessary
                ...
                for (i = 0; i < 12; i++) {
                    ...
                    if (is_disabled(selected_year - 0+ year)) class_name += ' dp_disabled';

                    // else, if a date is already selected and this is that particular year, highlight it
                    else if (default_year && default_year === selected_year - 0 + year) class_name += ' dp_selected';

                    // else, if this is the current system year, highlight it
                    else if (current_system_year === (selected_year - 0 + year)) class_name += ' dp_current';

                    // first three letters of the month's name
                    // html += '<td' + ($.trim(class_name) !== '' ? ' class="' + $.trim(class_name) + '"' : '') + '>' + (selected_year - 7 + year) + '</td>';
					html += '<td' + ($.trim(class_name) !== '' ? ' class="' + $.trim(class_name) + '"' : '') + '>' + (selected_year - 0 + year) + '</td>';
                }
